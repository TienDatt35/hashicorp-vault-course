#!/usr/bin/env bash
# verify.sh — kiểm tra bài thực hành: Khám phá trạng thái Replication trên Vault OSS
#
# Quy ước:
#   pass "mô tả ngắn"  -> in dòng [PASS]
#   fail "mô tả ngắn"  -> in dòng [FAIL] và tăng số lỗi
#
# Lưu ý quan trọng: Vault OSS luôn trả về mode=disabled cho cả DR lẫn
# Performance Replication — đây là hành vi đúng, không phải lỗi.
#
...
echo "Đang kiểm tra bài thực hành — Khám phá trạng thái Replication trên Vault OSS"
echo

# --- Kiểm tra 0: Vault có thể truy cập -------------------------------------------
# Tiên quyết: Vault dev server phải đang chạy trong Codespace.
...
# --- Kiểm tra 1: sys/replication/status trả về response hợp lệ (Bước 1) ----------
# Lệnh vault read sys/replication/status phải thành công (exit code 0).
...
# --- Kiểm tra 2: Response có chứa thông tin về DR (Bước 1 + Bước 2) --------------
# Output phải chứa field dr.mode — xác nhận API trả về cấu trúc đúng.
...
# --- Kiểm tra 3: Response có chứa thông tin về Performance (Bước 1 + Bước 2) -----
# Output phải chứa field performance.mode.
...
# --- Kiểm tra 4: dr.mode là 'disabled' (Bước 2) ----------------------------------
...
DR_MODE=$(vault read -format=json sys/replication/status 2>/dev/null \
  | grep -o '"dr\.mode":"[^"]*"' | cut -d'"' -f4)
...
# --- Kiểm tra 5: performance.mode là 'disabled' (Bước 2) -------------------------
# OSS Vault luôn trả về disabled cho Performance mode — đây là hành vi đúng.
PERF_MODE=$(vault read -format=json sys/replication/status 2>/dev/null \
  | grep -o '"performance\.mode":"[^"]*"' | cut -d'"' -f4)
...
# --- Kiểm tra 6: sys/replication/dr/status trả về response hợp lệ (Bước 2) ------
# Endpoint chi tiết cho DR phải trả về exit code 0.
...
# --- Kiểm tra 7: sys/replication/performance/status trả về response hợp lệ (Bước 2) -
# Endpoint chi tiết cho Performance phải trả về exit code 0.
